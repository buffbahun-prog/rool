...
import { DataPayloadType, DataType } from "../types.js";
...
// }
export function encodeDataPayload(dataPayload) {
...
export function encodeStatusMessage(status) {
...
export function decodeStatusMessage(packet) {
    const type = packet[0];
...
//   return list;
// }
export function encodeChunkPayload(chunkPayload) {
...
export function decodeChunkPayload(buffer) {
    const IV_LEN = 12;
    let offset = 0;
...
export function encodeChunkAck(chunkAck) {
...
export function decodeChunkAck(buffer) {
    let offset = 0;
    const fileId = buffer[offset];
    offset += 1;
    const chunkId = getUint32BE(buffer, offset);
...
export function encodePauseStatus(pauseStatus) {
...
export function decodePauseStatus(buffer) {
    let offset = 0;
...
export function setUint32BE(array, offset, value) {
    array[offset] = (value >>> 24) & 0xFF;
    array[offset + 1] = (value >>> 16) & 0xFF;
    array[offset + 2] = (value >>> 8) & 0xFF;
    array[offset + 3] = value & 0xFF;
}
export function getUint32BE(array, offset) {
...
export function setBigUint64BE(array, offset, value) {
    for (let i = 7; i >= 0; i--) {
        array[offset + i] = Number(value & 0xffn);
        value >>= 8n;
    }
}
export function getBigUint64BE(array, offset) {
    let value = 0n;
    for (let i = 0; i < 8; i++) {
...
export async function buildRoot(leafHashes) {
    let currentLevel = [];
...
export async function getMerkleProof(leafHashes, index) {
    let nodes = [];
    for (let i = 0; i < leafHashes.length; i += 32) {
...
export async function verifyChunk(rootHash, chunkHash, flatProof, index) {
