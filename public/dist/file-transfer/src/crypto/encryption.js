...
// }
export async function encryptJustChunk(aesKey, data) {
    const iv = crypto.getRandomValues(new Uint8Array(12));
    const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, aesKey, new Uint8Array(data));
    return { ciphertext, iv };
}
// ------------------- RECEIVE FUNCTION -------------------
export async function decryptVerifyDecompress(recipientEncryptionKey, payload) {
    const { ciphertext, iv, ephemeralPublicKey } = payload;
    // ---------------- IMPORT EPHEMERAL KEY ----------------
    const importedEphemeralKey = await crypto.subtle.importKey("spki", ephemeralPublicKey, { name: "ECDH", namedCurve: "P-256" }, true, []);
    // ---------------- DERIVE AES KEY ----------------
    const aesKey = await crypto.subtle.deriveKey({ name: "ECDH", public: importedEphemeralKey }, recipientEncryptionKey.privateKey, { name: "AES-GCM", length: 256 }, false, ["decrypt"]);
    // ---------------- DECRYPT ----------------
    try {
        const decryptedCompressed = await crypto.subtle.decrypt({ name: "AES-GCM", iv: new Uint8Array(iv) }, aesKey, ciphertext);
...
export function uint8ArrayToBlob(array, type = "application/octet-stream") {
