...
import { formatFileSize, formatTimeLeft, getFileCategory, } from "./utils/convert.js";
...
const peerPauseBar = document.getElementById("peerPauseBar");
const peerPauseTextElm = document.getElementById("peerPauseText");
...
function applyRemotePause(isPause, by) {
...
//   return receiver;
// }
async function setupFinalDownload(filesInfo, opfsRoot) {
...
let peerType = null;
let viewPage = ViewPage.TransferLanding;
const localInitialView = document.getElementById("local");
// const globalInitialView = document.getElementById("global") as HTMLDivElement;
const roomJoinView = document.getElementById("qrExchangeCard");
const roomJoinViewChooseCont = document.getElementById("qrExchangeChoose");
const downloadBtn = document.getElementById("downloadBtn");
const genRoomHeading = document.getElementById("showQr");
const joinRoomHeading = document.getElementById("scanQr");
...
const transferLandingView = document.getElementById("transferLanding");
const localCardFileUploadView = document.getElementById("localCardFileUpload");
const sendFileLocalBtn = document.getElementById("sendFileLocal");
const recieveFileLocalBtn = document.getElementById("recieveFileLocal");
const localFileUploadBackBtn = document.getElementById("localFileUploadBackBtn");
...
const localFileUploadBtn = document.getElementById("localFileDrpzn");
...
async function updateLocalFileView() {
    if (fileHandles.length > 0) {
      ...
      `;
            const removeBtn = liElement.querySelector(".file-rmv-btn");
...
const previewFileListCont = document.getElementById("previewFileList");
const previewFilesSummary = document.getElementById("previewFilesSummary");
const previewVolSummary = document.getElementById("previewVolSummary");
let removeFileIds = [];
function updatePreviewPage(filesInfo) {
    ...
    `;
        ...
        removeBtn?.addEventListener("click", (e) => {
            e.stopPropagation();
            if (removeFileIds.includes(info.fileId))
                removeFileIds = removeFileIds.filter(id => id !== info.fileId);
            else
                removeFileIds.push(info.fileId);
            updatePreviewPage(filesInfo);
        });
        previewFileListCont.appendChild(liElement);
    }
}
const localFileUploadNextBtn = document.getElementById("localFileUploadNextBtn");
...
const qrExchangeBackBtn = document.getElementById("qrExchangeBackBtn");
...
const createRoomView = document.getElementById("showQrCard");
const joinRoomView = document.getElementById("scanQrCard");
...
const fileTransferView = document.getElementById("localCardFileTransfer");
const transferCancleBtn = document.getElementById("transferCancleBtn");
...
let recieverWS = null;
let senderWRTC = null;
let recieverWRTC = null;
pauseToggleBtn.addEventListener("click", () => {
    if (senderWRTC)
        senderWRTC.onTransferPause();
    if (recieverWRTC)
        recieverWRTC.onTransferPause();
});
createRoomBtn?.addEventListener("click", async () => {
    ...
    if (infoBarText)
        infoBarText.textContent = "Creating Room";
    if (infoBar)
        infoBar.classList.add("show-info");
    ...
    senderWS.on("roomCreate", (payload) => {
        ...
        if (!code) {
            if (infoBarText)
                infoBarText.textContent = "Failed Retry";
    ...
    senderWS.on("roomJoining", () => {
        if (infoBarText)
            infoBarText.textContent = "Joining Room";
    });
    ...
    senderWRTC.on("fileInfo", (payload) => {
        ...
        fileProgressElms.forEach((fpe) => {
            if (!fpe.elms)
                throw new Error("File card not initialized properly");
    ...
    senderWRTC.on("progress", (payload) => {
        ...
        if (!filePrgAnim || !filePrgElm)
            throw new Error("No file with fileId: " + fileId + " found");
        if (!filePrgElm.elms)
            throw new Error("no progress percentage element found for fileId: " + fileId);
    ...
    senderWRTC.on("transferPause", (payload) => {
        ...
        if (by === "remote")
            applyRemotePause(paused, "reciever");
        else {
...
roomJoinBtn?.addEventListener("click", async () => {
    ...
    if (infoBarR)
        infoBarR.classList.add("show-info");
    if (infoBarTextR)
        infoBarTextR.textContent = "Joining Room";
    ...
    recieverWS.on("roomJoin", async () => {
        if (infoBarTextR)
            infoBarTextR.textContent = "Joined Room";
    });
    ...
    let fileProgressElms = [];
    let fileProgressAnims = [];
    const selectedFileIdsForDwnld = new Map();
    ...
    recieverWRTC.on("isHashing", (payload) => {
        ...
        if (!filePrgElm?.elms)
            throw new Error("no progress percentage element found for fileId: " + payload.fileId);
...
function privacyPopover(safetyNum) {
    const numDisplayElm = document.getElementById("safetyNum");
    if (!popoverElm || !numDisplayElm)
        throw new Error("Privacy popover element not on the DOM");
...
copyCodeBtn?.addEventListener("click", () => {
    ...
    if (code)
        copyToClipBoard(`https://rool.buffbahun.workers.dev/file-transfer/?roomCode=${code}`);
});
...
async function shareSecurityCode(code) {
    try {
...
async function shareRoomUrl(code) {
    try {
        await navigator.share({
            title: "Room Join URL",
...
function popoverUI(type, message) {
    ...
    if (!errorPopoverElm || !errorMessageElm || !errorPopTitle)
        return;
...
function createFileCard(fileMeta) {
    ...
    const template = document.getElementById("pieProgressTemplate");
    if (!template || !parentElm)
        return;
    const clone = template.content.cloneNode(true);
    const filterElm = clone.querySelector(".shadowFilter");
    const filterId = `shadowFilter${fileId}`;
    filterElm.id = filterId;
    const circleElm = clone.querySelector(".bgTrack");
    // circleElm.setAttribute("filter", `url(#${filterId})`);
    const pathElm = clone.querySelector(".progressWedge");
    pathElm.setAttribute("filter", `url(#${filterId})`);
    pathElm.id = `progressWedge${fileId}`;
    const uploadStatusElm = clone.querySelector(".uploadStatus");
    uploadStatusElm.id = `uploadStatus${fileId}`;
    const fileNameElm = clone.querySelector(".fileNameTxt");
    fileNameElm.textContent = fileName;
    const fileSizeElm = clone.querySelector(".fileSizeTxt");
    fileSizeElm.textContent = formatFileSize(fileSize, 1);
    const fileTypeElm = clone.querySelector(".fileTypeTxt");
    fileTypeElm.textContent = getFileCategory(undefined, fileType) ?? "Unknown Type";
    const progressSpeedElm = clone.querySelector(".progressSpeed");
    progressSpeedElm.id = `progressSpeed${fileId}`;
    const progressTimeElm = clone.querySelector(".progressTime");
    progressTimeElm.id = `progressTime${fileId}`;
    const progressPercentageElm = clone.querySelector(".progressPercentage");
    progressPercentageElm.id = `progressPercentage${fileId}`;
    const pauseBtnElm = clone.querySelector(".pauseBtn");
    pauseBtnElm.id = `pauseBtn${fileId}`;
    const pauseBtnTxtElm = clone.querySelector(".pauseBtnTxt");
    pauseBtnTxtElm.id = `pauseBtnTxt${fileId}`;
    const dwnldBtnElm = clone.querySelector(".dwnldBtn");
    dwnldBtnElm.id = `dwnldBtn${fileId}`;
    const selectButtonElm = clone.querySelector(".sel");
...
function smoothProgressTo(targetAngle, animMap, reset = false) {
    if (reset) {
    ...
    function renderFrame(currentTime) {
        ...
        if (frameAngle >= 2 * Math.PI)
            frameAngle = 2 * Math.PI - 0.001;
        if (frameAngle <= 0)
            frameAngle = 0.0001;
        ...
        if (!animMap.arcPath)
            throw new Error("Progress arch svg path not found for fileId: " + animMap.fileId);
