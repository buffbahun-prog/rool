...
export function charCount(textCont) {
    const segmenter = new Intl.Segmenter(undefined, {
        granularity: "grapheme"
    });
    return [...segmenter.segment(textCont)].length;
}
export function wordCount(textCont) {
    const segmenter = new Intl.Segmenter(undefined, {
        granularity: "word"
    });
...
export function toSentenceCase(textCont) {
    return textCont
        .toLocaleLowerCase()
        .replace(/(^\s*\p{L}|[.!?]\s*\p{L})/gu, m => m.slice(0, -1) + m.slice(-1).toLocaleUpperCase()).replace(/\s+[i]\s+/gu, m => m.toLocaleUpperCase());
}
export function toLowerCase(textCont) {
    return textCont
        .toLocaleLowerCase();
}
export function toUpperCase(textCont) {
    return textCont
        .toLocaleUpperCase();
}
export function toCapitalizedCase(textCont) {
    return textCont
        .toLocaleLowerCase()
        .replace(/\b\p{L}/gu, char => char.toLocaleUpperCase());
}
export function toTitleCase(textCont) {
    let wordIndex = 0;
    const words = [...textCont.matchAll(/\p{L}+/gu)];
    const totalWords = words.length;
    return toSentenceCase(textCont).replace(/\p{L}+/gu, word => {
...
export function toAlternatigCase(textCont) {
    let upper = false;
    const segmenter = new Intl.Segmenter(undefined, {
        granularity: "grapheme"
    });
...
export function toInverseCase(textCont) {
    const segmenter = new Intl.Segmenter(undefined, {
        granularity: "grapheme"
    });
    return [...segmenter.segment(textCont)].map((char) => {
