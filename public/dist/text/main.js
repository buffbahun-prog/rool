...
function main() {
    if (pathSegments.length === 2 && pathSegments[0] === "text" && textUtilitySubPaths.includes(pathSegments[1])) {
        ...
        if (consideringTextareaElm)
            updateActionUI(consideringTextareaElm, allTextAreaElms);
...
main();
function updateCountUI(textElm) {
...
function popoverUI(type, message) {
    if (type === "error") {
        ...
        if (!errorPopoverElm || !errorMessageElm)
            return;
...
function updateDummyGeneratorUI(el, type, count, lang) {
    let dummyText = "";
    if (type === "paragraph") {
...
function updateActionUI(consideringTextareaElm, allTextAreaElms) {
