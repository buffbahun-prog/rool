import "./components/barcode/barcode.js";
import "./components/qrcode/qrcode.js";
const errSelect = document.getElementById("errorcr");
const qrInput = document.getElementById("qrdata");
const qrElm = document.getElementById("qr");
if (qrElm && errSelect && qrInput) {
    errSelect.addEventListener("change", () => {
        const errVal = errSelect.value;
        console.log(errVal);
...
if (downloadBtn) {
    downloadBtn.addEventListener("click", () => {
        if (!qrInput?.value || !qrElm?.shadowRoot)
            return;
        const svgElm = qrElm.shadowRoot.querySelector("svg");
...
//   })
function downloadSvgAsFile(svgElm, filename = "rool_qr", format) {
    if (!svgElm)
        return;
    if (format === "svg") {
...
function convertSvgToRaster(svgElement, filename = "rool_qr", format) {
