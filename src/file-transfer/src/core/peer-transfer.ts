...
import { DataType, FileMetadata, FileTransferState, PeerType, TransferState, WorkerAction, type TransferEvents, type WorkerRequest, type WorkerResponse } from "../types";
import { isBitmapComplete } from "../utils/convert";
import { TypedEmitter } from "./emmiter";
import { getBigUint64BE, getUint32BE, setBigUint64BE, setUint32BE } from "./transfer";
...
export class PeerTransfer extends TypedEmitter<TransferEvents> {
  ...
  private workersFileMap: {worker: Worker, fileId: number}[] = [];

  private transferState: TransferState;

  private fileTransferState: Map<number, {
    currentState: FileTransferState;
    prevState: FileTransferState;
  }> = new Map();

  private static readonly MAX_BUFFER = 8 * 1024 * 1024;
  private static readonly MIN_BUFFER = 4 * 1024 * 1024;

  private static readonly CHUNK_SIZE = 128 * 1024;

  private localEncKeys: CryptoKeyPair | null = null;
  private remoteEncPublicKey: CryptoKey | null = null;

  private localSigKeys: CryptoKeyPair | null = null;
  private remoteSigPublicKey: CryptoKey | null = null;

  private localPrivacyConfirmed = false;
  private remotePrivacyConfirmed = false;

  private unwrappedLocalTransferKey: CryptoKey | null = null;
  private unwrappedRemoteTransferKey: CryptoKey | null = null;

  private fileHandleList: FileSystemFileHandle[] | File[];

  private fileMetadataList: FileMetadata[] | null = null;

  // fileId, boolean
  private retryChunksAck: Map<number, boolean> = new Map();
  private filesHashAck: Map<number, boolean> = new Map();
  private localFilePause: Map<number, boolean> = new Map();
  private remoteFilePause: Map<number, boolean> = new Map();
//   private chunkIdWrittenTo: Map<number, number> = new Map();

  private remoteFileHash: Map<number, string> = new Map();
  private localFileHash: Map<number, string> = new Map();

  private localTransferPause: boolean;
  private remoteTransferPause: boolean;


  publicKeysRecievedAck = false;
  privacyConfirmedAck = false;
  transferKeyRecievedAck = false;
  fileMetadataRecievedAck = false;
  ...
  constructor(peerType: PeerType, fileList: FileSystemFileHandle[] | File[]) {
    super();
  ...
  async initWRTC() {
    try {
        if (!this.pc) throw new Error("WebRTC nor initialized properly.");
  ...
  private dataChannelEvents() {
    if (!this.dataChannel) throw new Error("Data channels not initialized properly");
  ...
  async addRemoteCandidate(candidates: string | string[]) {
    ...
    for (const c of list) {
        ...
        const exists = this.pendingCandidates.some(
            x => x.candidate === candidateObj.candidate
        );
  ...
  private isPaused(fileId: number) {
    return (
        this.localTransferPause === true
  ...
  private onLocalPause(pauseType: DataType.TransferPause | DataType.FilePause, fileId?: number) {
  ...
  private async onRemotePause(packet: Uint8Array) {
    if (!this.fileMetadataList || this.fileMetadataList.length <= 0)
        throw new Error("file metadata list not initialized properly");

    let offset = 0;
    const pauseType: DataType.TransferPause | DataType.FilePause = packet[offset];
    offset++;
    ...
    if (pauseType === DataType.TransferPause) {
        if (isPaused === this.remoteTransferPause) return;
        ...
        if (isPaused === this.remoteFilePause.get(fileId)) return;
  ...
  private getFileIdWorker(fileId: number) {
  ...
  private async initWorkers() {
    if (!this.fileMetadataList || this.fileMetadataList.length <= 0) throw new Error("File Metadata list not initialized properly.");
    ...
    let initWrittenFileIds: boolean[] = new Array(this.workersFileMap.length).fill(false);

    this.workersFileMap.forEach(async (wfm) => {
        if (!this.unwrappedLocalTransferKey || !this.unwrappedRemoteTransferKey) throw new Error("local, remote transfer keys not properly initialized for workers");
        const {worker, fileId} = wfm;

        const fileMetadataForId = this.fileMetadataList?.find((fml) => fml.fileId === fileId);
        if (!fileMetadataForId) throw new Error(`Cant find metadata for file with id: ${fileId}`);
            ...
            switch (action) {
                case WorkerAction.InitEncryption: {
                    if (status === "SUCCESS" && result) {
                        ...
                        const nextChunkReq: WorkerRequest = {
                ...
                case WorkerAction.GetNextChunk: {
                    if (status === "SUCCESS" && result) {
                        ...
                        if (packet) {
                            ...
                            () => {
                                const bufferPauseReq: WorkerRequest = {
                                ...
                                const bufferResumeReq: WorkerRequest = {
                ...
                case WorkerAction.GetFileHash: {
                    if (status === "SUCCESS" && result) {
                        ...
                        if (this.peerType === PeerType.Sender) {
                            if (!this.localSigKeys || !this.unwrappedLocalTransferKey) throw new Error("local signature keys and/or local transfer key not initialized properly");
                            ...
                            if (isValid) {
                                const getFileDwnldReq: WorkerRequest = {
        ...
        const remoteTransferKeyReq: WorkerRequest = {
        ...
        if (this.peerType === PeerType.Sender) {
            ...
            if (!fileHandle) throw new Error(`Cant find fileHandle for file id: ${fileId}`);
            const file = (fileHandle instanceof FileSystemFileHandle) ? await fileHandle.getFile() : fileHandle;

            const metadataReq: WorkerRequest = {
            ...
            const initWritableReq: WorkerRequest = {
  ...
  private onUpdateTransferState(to: TransferState) {
    if (this.transferState === to) return;
    switch (to) {
        ...
        case TransferState.FileTrasnferStart:
            if (!this.fileMetadataList || this.fileMetadataList.length <= 0) return;
  ...
  private async onSendData(dataType: DataType, packet?: Uint8Array, fileId?: number) {
    switch (dataType) {
  ...
  private async onRecieveData(payload: Uint8Array) {
    ...
    switch (dataType) {
        ...
        case DataType.FileChunk: {
            ...
            if (worker) {
                ...
                const setChunkReq: WorkerRequest = {
        ...
        case DataType.ChunkSendComplete: {
            ...
            const worker = this.getFileIdWorker(fileId);
            if (worker) {
                const getBitmapReq: WorkerRequest = {
        ...
        case DataType.RetryChunks: {
            ...
            if (this.retryChunksAck.get(fileId)) return;
            this.retryChunksAck.set(fileId, true);

            const worker = this.getFileIdWorker(fileId);
            if (!worker) throw new Error("no worker with fileId: " + fileId + " found.");

            if (isBitmapComplete(bitmap)) {
                const hashReq: WorkerRequest = {
        ...
        case DataType.TransferPauseAck: {
            if (this.transferPauseAck) return;
        ...
        case DataType.FilePauseAck: {
            ...
            if (this.filePauseAck.get(fileId)) return;
  ...
  private async sendLocalPublicKeys() {
    if (!this.localEncKeys) this.localEncKeys = await generateEncryptionKeyPair();
    if (!this.localSigKeys) this.localSigKeys = await generateSigningKeyPair();
  ...
  private async onPublicKeysRecieved(payload: Uint8Array) {
    let offset = 0;
    const encKeyLen = 294;
    const sigKeyLen = 91;
    if (payload.length !== encKeyLen + sigKeyLen) throw new Error("public keys payload arrived missmatched");
  ...
  private async displaySafetyNum(remotePubKeys: Uint8Array) {
    if (!this.localEncKeys || !this.localSigKeys) throw new Error("Public keys not initialized properly");
  ...
  onPrivaryPopoverConfirm(isSecure: boolean) {
    this.onUpdateTransferState(TransferState.PrivacyConfirm);
  }

  private sendPrivacyConfirm() {
  ...
  private onPrivacyConfirmRecieve() {
  ...
  private async sendWrappedLocalTransferKey() {
    if (!this.remoteEncPublicKey) throw new Error("remote encryption public keys not found");
  ...
  private async onWrappedTransferKeyRecieved(payload: Uint8Array) {
    if (!this.localEncKeys) throw new Error("Local encryption keys not initialized.");
  ...
  private async sendFileListMetadata() {
    ...
    if (!this.localSigKeys || !this.unwrappedLocalTransferKey) throw new Error("local signing keys and local transfer key not initialized properly");
  ...
  private async onFileListMetadataRecieve(payload: Uint8Array) {
    if (!this.remoteSigPublicKey || !this.unwrappedRemoteTransferKey) throw new Error("Remote Signature public key or/and remote transfer key not initialized properly");
  ...
  private async sendAckMessage(ackDataType: DataType.PublicEncrytionAndSigningKeyRecievedAck | DataType.TransferKeyAck | DataType.PrivacyConfirmAck | DataType.FileListMetadataAck) {
  ...
  private async onFileHashRecieve(payload: Uint8Array) {
    const outFileId = payload[0];
    if (this.remoteFileHash.get(outFileId)) return;

    try {
        if (!this.remoteSigPublicKey || !this.unwrappedRemoteTransferKey) throw new Error("remote public signature key and/or remote transfer key not initialized properly.");
  ...
  private async sendUntilAckTrue(payload: Uint8Array, checkAck: () => boolean) {
    if (!this.dataChannel) throw new Error("Data channels not initialized properly");
    if (payload.byteLength <= 0) throw new Error("Data payload empty");
    let retries = 0;
    const MAX_RETRIES = 20;
    while (retries < MAX_RETRIES) {
      if (checkAck()) return;
      if (this.dataChannel.readyState !== "open") throw new Error("Connect not open");
...
function encodePublicKeys(publicEncKeyBuff: Uint8Array, publicSigKeyBuff: Uint8Array): Uint8Array {
...
function encodeWrappedTransferKey(wrappedTransferKey: Uint8Array): Uint8Array {
...
export async function encodeAndEncryptFileMatadataWithSignature(fileMetadataList: FileMetadata[], localSigPrivateKey: CryptoKey, localTransferKey: CryptoKey) {
...
export async function verifyDecryptAndDecodeFileMetadata(payload: Uint8Array, remoteSigPubKey: CryptoKey, unwrappedRemoteTransferKey: CryptoKey) {
  ...
  if (!isVaild) throw new Error("The file metadata not valid");
...
export function encodeChunksBitmap(fileId: number, totalChunks: number, bitmap: Uint8Array) {
...
export function decodeChunksBitmap(packet: Uint8Array) {
    let offset = 0;
...
export async function encodeAndEncryptFileHashWithSignature(fileId: number, hash: string, localSigPrivateKey: CryptoKey, localTransferKey: CryptoKey) {
    const stringEncoder = new TextEncoder();
...
export async function verifyDecryptAndDecodeFileHash(payload: Uint8Array, remoteSigPubKey: CryptoKey, unwrappedRemoteTransferKey: CryptoKey) {
  // signature 64 bytes and iv 12 bytes
  let offset = 0;
  const signatureLen = 64;
  const ivLen = 12;
  ...
  if (!isVaild) throw new Error("The file hash not valid");
  ...
  if (outFileId !== fileId) throw new Error("file Ids not matches on packet");
