...
export class RecieverWS extends TypedEmitter<WSEvents> {
  ...
  private sendToServer(type: WSMessageType, msg: string) {
    try {
      ...
      switch (type) {
        case "answer":
          if (!msg) throw new Error("answer message empty");
          const answerMsg: WSMessageEvent = {
            type: type,
            answer: msg
          }
        ...
        case "reciever-candidates":
          if (!msg) throw new Error("reciever candidates message empty");
          const recieverCandidatesMsg: WSMessageEvent = {
            type: type,
    ...
    try {
      switch (msg.type) {
        case WSMessageType.Init:
          if (!msg.offer) throw new Error("offer from server is empty.");
          if (!msg.senderCandidates) throw new Error("sender candidates from server is empty.");
  ...
  sendRecieverCandidates(candidates: string) {
