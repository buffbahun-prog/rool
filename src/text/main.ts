    ...
    inputTextareaElm.addEventListener("input", () => {
      ...
      if (clearBtnElm) {
        clearBtnElm.addEventListener("click", () => {
            ...
            if (outputTextareaElm) outputTextareaElm.value = "";
            if (characterCountElm) characterCountElm.textContent = "0";
            if (wordCountElm) wordCountElm.textContent = "0";
            if (lineCountElm) lineCountElm.textContent = "0";
        });
      }
    }); 
...
function updateCountUI(el: HTMLElement, value: string) {
