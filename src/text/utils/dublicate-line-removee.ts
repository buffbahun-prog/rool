import { isEmptyLine } from "./shared";

export function removeDuplicateLines(textCont: string, caseSensitive: boolean, trimWhitespace: boolean, emptyLinesRemove: boolean) {
    ...
    const lines = textCont.split("\n").map(line => {
        ...
        if (emptyLinesRemove && (!line || isEmptyLine(line))) return false;
        else return true;
    });
    ...
    for (let i = 0; i < lines.length; i++) {
        if (duplicateIndex.includes(i)) continue;
        const line1 = lines[i];
        ...
        for (let j = i + 1; j < lines.length; j++) {
            if (duplicateIndex.includes(j)) continue;
            const line2 = lines[j];
...
function isCharsIdentical(collator: Intl.Collator, char1: string, char2: string) {
    return collator.compare(char1, char2) === 0; 
}

function isLinesIdentical(collator: Intl.Collator, line1: string[], line2: string[]) {
    if (line1.length !== line2.length) return false;
    for (let i = 0; i < line1.length; i++) {
        const char1 = line1[i];
        const char2 = line2[i];
        if (!isCharsIdentical(collator, char1, char2)) return false;
    }
